          ...
          <q-card-section style="flex-wrap: wrap" horizontal class="row">
            ...
            <q-card-section class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
              ...
              <div v-if="!is_own">
                ...
                <q-btn v-if="!fullInfo" color="primary" :to="{name:'order_info',params: { order_slug:order.name_slug }}" label="Предложить технику"/>
              </div>
              <div v-else>
                ...
                <q-btn class="q-mr-sm" v-if="!is_apply" @click="$router.push({name:'profile_order_info',params:{order_slug:order.name_slug}})" outline color="primary" label="Открыть"/>
                <q-btn class="q-mr-sm" v-else @click="$router.push({name:'profile_apply_info',params:{order_slug:order.name_slug}})" outline color="primary" label="Детали"/>
                <q-btn v-if="!order.worker" color="primary" @click="deleteOrder(order.id)" label="Удалить"/>
              </div>
